...
printf("sum=%d\n",sum);
嵌套循环 各种循环语句之间可以相互嵌套
breake出现在switch里是跳出switch语句 在循环里是跳出循环语句执行后面代码 在嵌套循环里是跳出最近的内循环语句执行后面的代码

11.3
这节课讲到了整形与字符 整形与单精度浮点型的关系
强制转换 隐性转换  （int）x     对于隐性转换赋值号左边的精度要大于右边的精度否则就会被舍掉
字符与整形的关系 可用用int表示ASCLL码也可以用char表示字符，字符和整形之间是可用使用运算符号，赋值则是从右至左，而运算是从左至右；
对于整型的除法会自动舍去小数，除非被除数或者除数有小数点也可用用强制转换，除号整形自动向下取整，处余两边必须要为整型。即整数之间的除法会自动取整为整数。
